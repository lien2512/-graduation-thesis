...
ngOnInit(): void {
    ...
    this.calendarData = this.userInfo.calendar;
...
generateCalendar() {
  ...
  async fillCalendar() {
    ...
    const weekNumber = moment(calendarStartDate).format('YYYY') + moment(calendarEndDate).isoWeek();
        // const dataBee: any = await this.firebaseService.getRefById('users',this.userInfo.id);
    ...
    //     if (response.code == 200) {
        // console.log(dataBee.calendar);
            const listOrder = this.calendarData.order;
            ...
            // Available Time
            const availableRange = this.calendarData.available_time2;
