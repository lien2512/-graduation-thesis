...
export class AccSettingComponent implements OnInit {
  ...
  @ViewChild('fullcalendarPandaProfileSetting') fullcalendar: FullCalendarComponent;
      ...
      setTimeout(() => {
        this.calendar = {
          ...
          eventDidMount: (info) => {
            ...
            node.setAttribute('style', 'position: absolute; z-index: 9; top: 5px; bottom: 5px; right: 5px; color: #fff; cursor: pointer');
  ...
  handleSelect(event) {
    ...
    if (this.calendarEvent.length > 0) {
      ...
      for (let index = 0; index < this.calendarEvent.length; index++) {
        const elementStartTime = moment(this.calendarEvent[index].start).format('X');
        const elementEndTime = moment(this.calendarEvent[index].end).format('X');
        const elementStartDate = moment(this.calendarEvent[index].start).format('DD');
        if (eventStartTime >= elementStartTime && eventStartTime < elementEndTime) {
          if (eventStartDate == elementStartDate) {
            if (sameEventId != -1) {
              this.calendarEvent[sameEventId].start = moment(this.calendarEvent[index].start).format('YYYY-MM-DD HH:mm:ss');
              this.calendarEvent.splice(index, 1);
              index--;
            } else {
              this.calendarEvent[index].end = moment(event.end).format('YYYY-MM-DD HH:mm:ss');
              sameEventId = index;
            }
          }
          ...
          if (eventStartTime == elementStartTime && eventEndTime < elementEndTime) {
            this.calendarEvent[index].end = moment(event.end).format('YYYY-MM-DD HH:mm:ss');
            pushEvent = false;
          }
          if (eventEndTime > elementEndTime && eventStartDate == elementStartDate) {
            if (sameEventId != -1) {
              this.calendarEvent[sameEventId].start = moment(this.calendarEvent[index].start).format('YYYY-MM-DD HH:mm:ss');
              this.calendarEvent.splice(index, 1);
              index--;
          ...
          if (eventEndTime > elementEndTime) {
            if (checkDuplicateEvent == false) {
              this.calendarEvent[index].end = moment(event.end).format('YYYY-MM-DD HH:mm:ss');
              this.calendarEvent[index].start = moment(event.start).format('YYYY-MM-DD HH:mm:ss');
              checkDuplicateEvent = true;
              pushEvent = false;
            } else {
              this.calendarEvent.splice(index, 1);
          ...
          if (eventEndTime >= elementStartTime && eventEndTime <= elementEndTime) {
            if (sameEventId == -1) {
              this.calendarEvent[index].start = moment(event.start).format('YYYY-MM-DD HH:mm:ss');
              sameEventId = index;
              pushEvent = false;
              ...
              this.calendarEvent[sameEventId].end = moment(this.calendarEvent[index].end).format('YYYY-MM-DD HH:mm:ss');
              this.calendarEvent.splice(index, 1);
              index--;
            }
          }
  ...
  _generateCalendar() {
    ...
    _events.forEach((element) => {
      ...
      if (dayStart != dayEnd || (moment(element.start).format('DD') != moment(element.end).format('DD') && dayStart == dayEnd)) {
        ...
        this.calendarEvent.push({
          ...
          end: moment(element.start).add(1, 'days').format('YYYY-MM-DD 00:00:00'),
          allDay: false,
          backgroundColor: '#B953BF',
          id: uuid.v4(),
          editable: true
        ...
        if (dayDiff == 1) {
          const _startTime = moment(element.start).add(1, 'days').format('YYYY-MM-DD 00:00:00');
          ...
          for (let i = 1; i <= dayDiff; i++) {
            let _endTime = moment(element.start).add((i + 1), 'days').format('YYYY-MM-DD 00:00:00');
            const _startTime = moment(element.start).add(i, 'days').format('YYYY-MM-DD 00:00:00');
            if (i == dayDiff) {
  ...
  async _loadAvailableTime() {
    const dataPRofile: any = await this.firebaseService.getRefById('users', this.userProfile.id);
    ...
    setTimeout(() => {
      ...
      //   if (response.code == 200) {
          ...
          const availableRangeOrigin = JSON.parse(JSON.stringify(calendar.available_time1));

          let _finalEvents = [];
          const _currentWeek = moment(calendarApi.getCurrentData().dateProfile.currentRange.start).week();
          const _currentYear = moment(calendarApi.getCurrentData().dateProfile.currentRange.end).year();
          ...
          availableRange.forEach((available) => {
            if (available.type == 1) {
              ...
              available.start_time = moment(String(_currentYear)).week(_currentWeek).weekday(_dowS).format('YYYY-MM-DD ' + moment(available.start_time).format('HH:mm'));
              available.end_time = moment(String(_currentYear)).week(_currentWeek).weekday(_dowE).format('YYYY-MM-DD ' + moment(available.end_time).format('HH:mm'));

              if (available.start_time > available.end_time) {
                available.start_time = moment(available.start_time).subtract(1, 'week').format('YYYY-MM-DD HH:mm');
              }
            }
          });
          ...
          availableRange.forEach((el) => {
            ...
            availableRangeOrigin.forEach((elOrigin, index1) => {
              if (el.id != elOrigin.id) {
                if (_yS == _yE && el.start_time != elOrigin.start_time && el.end_time == elOrigin.start_time) {
                  el.end_time = elOrigin.end_time;
                  availableRange.splice(index1, 1);
                } else if (el.start_time <= elOrigin.start_time && el.end_time >= elOrigin.end_time) {
                  _rmEls.push(availableRange.filter(_el => _el.id == elOrigin.id)[0].id);
                }
              }
            });
          });
          ...
          availableRange.forEach((available) => {
            if (!_rmEls.includes(available.id)) {
              ...
              } else if (_eT < moment(calendarStartDate).format('YYYY-MM-DD HH:mm')) {
          ...
          _finalEvents.forEach((el) => {
            if (moment(el.start).format('YYYY-MM-DD') == moment(el.end).format('YYYY-MM-DD')) {
              _finalEventsOrigin.forEach((subEl, index) => {
                if (el.start != subEl.start && el.end >= subEl.start && el.start <= subEl.start) {
                  el.end = subEl.end;
                  rmId.push(subEl.id);
  ...
  _generateCalendarTitle(next) {
    this.calendarEvent.map((evt) => {
      let _title;
      if ((moment(evt.start).format('DD') != moment(evt.end).format('DD') && moment(evt.start).format('HH:mm') == '00:00') || (moment(evt.start).format('HH:mm') == '00:00' && moment(evt.end).format('HH') == '23')) {
        _title = 'Cả ngày';
      } else {
  ...
  async updateAvailableTime() {
    ...
    this.calendarEvent.forEach((element) => {
      this.availableTime.push({
        start_time: moment.utc(moment(element.start)).format('YYYY-MM-DD HH:mm'),
        end_time: moment.utc(moment(element.end)).format('YYYY-MM-DD HH:mm')
      });
    });

    const calendarApi = this.fullcalendar.getApi();
    const weekNumber = moment().format('YYYY') + moment(calendarApi.getCurrentData().dateProfile.currentRange.start).week();
    ...
    let b = moment(item.start_time);
      ...
      for (let i = 0; i < hours; i++) {
        moment(start, 'YYYY-MM-DD HH:mm').add(15, 'minutes').format('YYYY-MM-DD HH:mm')
        let end = moment(start, 'YYYY-MM-DD HH:mm').add(15, 'minutes')
        let duration = {start_time: moment.utc(moment(start)).format('YYYY-MM-DD HH:mm'), end_time: moment.utc(end).format('YYYY-MM-DD HH:mm')};
        timer.push(duration);
        start = end;
      }
    })
    console.log(timer);
    let data: any = await this.firebaseService.getRefById('users',this.userProfile.id);
    ...
    firebase.firestore().collection('users').doc(this.userProfile.id).update('calendar', calendar);
